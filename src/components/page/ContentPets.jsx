    ...
    <div className="container ">
      <div className="row d-flex justify-content-around">
        {pets.reverse().map((pet, index) => {
          if (index < 4) {
            return (
              <div className="card col-3 m-1 border-0  p-0 text-white">
                <img src={`http://localhost:6557/${pet.img}`} className="card-img" alt="..."/>
                  <div className="card-img-overlay ">
                    <h5 className="card-title text-center text-bottom fs-2">{pet.header}</h5>
                  </div>
              </div>
            );
          }
